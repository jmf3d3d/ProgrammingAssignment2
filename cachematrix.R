...
makeCacheMatrix <- function(x = matrix()) {
  ...
  ## these are the methods for setting and getting the components of the cached matrix
      ...
      get <- function()    ##simply returns the original vector
...
## This function accesses the cached object created by makeCachedMatrix
...
cacheSolve <- function(x, ...) {
  ...
  ## first get the existing matrix
        inverted <- x$getinv()
        
        
  ## check to see if this has been inverted and cached already and simply return it if so
        if(!is.null(inverted)) {
